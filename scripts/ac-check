#!/usr/bin/env bash
# Проверка AC-трассируемости: у каждого критерия приёмки approved-спеки
# должен быть хотя бы один помеченный тест. Использование:
...
#
# Токен — AC-<NNN>-<K> (номер спеки-номер критерия, конвенция issue #171,
# skills/tdd) или, для обратной совместимости, голый AC-<K> без префикса
# спеки. Сопоставление — по литеральному совпадению строки токена между
# спекой и тестом, парсер не разбирает имя файла спеки: спека, чьё имя
# файла не начинается с числа, работает наравне с остальными, пока её
# собственные критерии и закрывающие их тесты используют один и тот же
# префикс. Ровно поэтому AC-<NNN>-<K> с разными NNN у двух спек не
# пересекается, даже если K совпал. Голый AC-<K> остаётся глобальным пулом
# (было единственным поведением до issue #171): у двух approved-спек с
# совпавшим номером критерия покрытие одной засчитывается и для другой —
# ac-check печатает предупреждение при каждом голом токене, но не падает,
# чтобы существующие проекты не мигрировали принудительно.
set -u
...
if [ "$complete_mode" -eq 0 ]; then
  pending=$(printf '%s' "$sections" | grep -oE 'AC-[0-9]+(-[0-9]+)?[[:space:]]*\(ждёт #[0-9]+\)' | grep -oE 'AC-[0-9]+(-[0-9]+)?' | sort -u)
fi

while IFS= read -r ac; do
  [ -n "$ac" ] && acs+=("$ac")
done < <(printf '%s' "$sections" | grep -oE 'AC-[0-9]+(-[0-9]+)?' | sort -u)

if [ "${#acs[@]}" -eq 0 ]; then
  exit 0
fi

# Голые токены — предупреждаем, не блокируем (issue #171). Число, а не
# список: сами номера неотличимы от списка непокрытых AC в том же выводе.
bare_count=0
for ac in "${acs[@]}"; do
  case "$ac" in
    AC-*-*) ;;
...
if [ "$bare_count" -gt 0 ]; then
  echo "ac-check: предупреждение: $bare_count голых AC-токенов без привязки к спеке (устаревшая конвенция, issue #171 — см. AC-<NNN>-<K> в skills/tdd)" >&2
fi
...
if [ "${#test_files[@]}" -gt 0 ]; then
  covered=$(grep -rhoE 'AC-[0-9]+(-[0-9]+)?' "${test_files[@]}" 2>/dev/null | sort -u)
fi
